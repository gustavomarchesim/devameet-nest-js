...
export class RoomService {
  ...
  async listObjectsByLink(link: string) {
    this.logger.debug(`listObjectsByLink - ${link}`);
    const meet = await this._getMeet(link);
    return await this.objectModel.find({ meet });
  }

  // Verficiador de colisão! - Desafio One:One
  async checkCollision(dto: UpdateUserPositionDto, link: string) {
    const objects = await this.listObjectsByLink(link);
    for (const object of objects) {
      if (dto.x === object.x && dto.y === object.y && !object.canWalk) {
        console.log('Objeto em frente!');
        return true;
      }
    }

    const users = await this.listUsersPositionByLink(link);
    for (const user of users) {
      if (user.userId !== dto.userId && dto.x === user.x && dto.y === user.y) {
        console.log('Usuário em frente!');
        return true;
      }
    }
